...
run:
	...
	sudo docker run -d --restart="always" -h local.rdf.glytoucan -p 81:8080  -v /opt/rdf.glytoucan/maven:/root/.m2 -v /mnt/jenkins/workspace/rdf.glytoucan-ws:/workspace -v /etc/localtime:/etc/localtime -w /workspace --name="rdf.glytoucan" maven:3.3.3-jdk-8 mvn -U spring-boot:run

runprod:
	sudo docker run -d --restart="always" -h local.rdf.glytoucan -p 81:80  -v /opt/prod.glytoucan/tmp:/tmp -v /opt/prod.glytoucan/maven:/root/.m2 -v /mnt/jenkins/workspace/prod.glytoucan:/workspace -v /opt/prod.glytoucan/upload:/upload -v /etc/localtime:/etc/localtime -w /workspace --name="beta.glytoucan" -e "google.oauth2.clientId=$(GOOGLE_OAUTH2_CLIENTID)" -e "google.oauth2.clientSecret=$(GOOGLE_OAUTH2_CLIENTSECRET)" -e "spring.mail.username=$(SPRING_MAIL_USERNAME)" -e "spring.mail.password=$(SPRING_MAIL_PASSWORD)" -e "SPRING_TRIPLESTORE_PASSWORD=$(SPRING_TRIPLESTORE_PASSWORD)" -e "spring.triplestore.password=$(SPRING_TRIPLESTORE_PASSWORD)" -e "SPRING_TRIPLESTORE_URL=$(SPRING_TRIPLESTORE_URL)" -e "spring.triplestore.url=$(SPRING_TRIPLESTORE_URL)" --link beta.stanza_bluetree:stanza.glytoucan.org aoki/rdf.glytoucan
