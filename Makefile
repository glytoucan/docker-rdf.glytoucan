...
rundev:
	...
	sudo docker run -d --restart="always" -h local.rdf.glytoucan -p 81:80  -v /opt/rdf.glytoucan/tmp:/tmp -v /opt/rdf.glytoucan/maven:/root/.m2 -v /home/aoki/workspace/rdf.glytoucan:/workspace -v /opt/rdf.glytoucan/upload:/upload -v /etc/localtime:/etc/localtime -w /workspace --name="beta.glytoucan" -e "google.oauth2.clientId=$(GOOGLE_OAUTH2_CLIENTID)" -e "google.oauth2.clientSecret=$(GOOGLE_OAUTH2_CLIENTSECRET)" -e "spring.mail.username=$(SPRING_MAIL_USERNAME)" -e "spring.mail.password=$(SPRING_MAIL_PASSWORD)" --link beta.stanza_bluetree:stanza.glytoucan.org aoki/rdf.glytoucan
