...
runprod:
	sudo docker run -d --restart="always" -h local.rdf.glytoucan -p 81:80  -v /opt/prod.glytoucan/tmp:/tmp -v /opt/prod.glytoucan/maven:/root/.m2 -v /mnt/jenkins/workspace/prod.glytoucan:/workspace -v /opt/prod.glytoucan/upload:/upload -v /etc/localtime:/etc/localtime -w /workspace --name="beta.glytoucan" -e "google.oauth2.clientId=$(GOOGLE_OAUTH2_CLIENTID)" -e "google.oauth2.clientSecret=$(GOOGLE_OAUTH2_CLIENTSECRET)" -e "SPRING_MAIL_USERNAME=$(SPRING_MAIL_USERNAME)" -e "SPRING_MAIL_PASSWORD=$(SPRING_MAIL_PASSWORD)" -e "SPRING_TRIPLESTORE_PASSWORD=$(SPRING_TRIPLESTORE_PASSWORD)" -e "SPRING_TRIPLESTORE_URL=$(SPRING_TRIPLESTORE_URL)" --link beta.stanza_bluetree:stanza.glytoucan.org aoki/rdf.glytoucan
